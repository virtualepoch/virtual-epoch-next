...
export const TorusGroup = ({
  ...
  useFrame(() => {
    ref1.current.rotation.x =
      hubLinkClicked | start ? 0 : isHub ? Math.PI * 0.25 : 0;
    ref1.current.rotation.y +=
      hubLinkClicked | start ? -0.2 : isHub ? 0 : -0.02;
    ref1.current.rotation.z +=
      hubLinkClicked | start ? 0.2 : isHub ? -0.001 : 0.02;

    ref2.current.rotation.x += 0;
    ref2.current.rotation.y += hubLinkClicked | start ? 0.1 : isHub ? 0 : 0.01;
    ref2.current.rotation.z +=
      hubLinkClicked | start ? 0.1 : isHub ? 0.001 : 0.01;
